...
def load_data_file(file_name: str, min_time_point: int = 0, max_time_point: int = 5000, *,
    ...
    for file in files:
        if file.endswith('.tif'):
            continue
    number_length = len(directory + '/' + file[:-4]) - len(file_prefix)

    time_point_number = 0
    positions_of_previous_time_point = dict()

    while os.path.exists(f"{file_prefix}{time_point_number:0{number_length}}.tif"):
        ...
        image = tifffile.imread(f"{file_prefix}{time_point_number:0{number_length}}.tif")
