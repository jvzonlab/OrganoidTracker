...
def find_compartment(experiment: Experiment, position: Position) -> CellCompartment:
    ...
    last_time_point_number = experiment.last_time_point_number() - experiment.division_lookahead_time_points
    return find_compartment_ext(experiment.positions, experiment.links, experiment.images.resolution(),
                                last_time_point_number, position)


def find_compartment_ext(positions: PositionCollection, links: Links, resolution: ImageResolution,
                         division_lookahead_time_points: int, position: Position) -> CellCompartment:
    """Finds the compartment of the cell. If a cell is non-dividing, but a cell close by is dividing, then the cell is
    considered to be in a dividing compartment."""
    if positions.last_time_point_number() - position.time_point_number() < division_lookahead_time_points:
    ...
    for nearby_position in nearby_position_finder.find_closest_n_positions(positions.of_time_point(
