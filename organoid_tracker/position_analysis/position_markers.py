...
def set_position_type(positions: PositionCollection, position: Position, type: Optional[str]):
    """Sets the type of the cell. Set to None to delete the cell type."""
    type_str = type.upper() if type is not None else None
    positions.set_position_data(position, "type", type_str)


def get_position_types(position_data: PositionCollection, positions: Set[Position]) -> Dict[Position, Optional[str]]:
...
def get_positions_of_type(positions: PositionCollection, requested_type: str) -> Iterable[Position]:
    """Gets all positions of the requested cell type."""
    requested_type = requested_type.upper()
    return (position for position, position_type in positions.find_all_positions_with_data("type")
...
def get_raw_intensity(positions: PositionCollection, position: Position) -> Optional[float]:
    """Gets the raw intensity of the position."""
    return positions.get_position_data(position, "intensity")
