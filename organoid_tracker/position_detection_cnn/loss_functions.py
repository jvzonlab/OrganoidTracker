...
from organoid_tracker.position_detection_cnn.custom_filters import blur_labels, disk_labels, distance_map, \
    _gaussian_kernel, get_edges, get_edges_with_bottom


# def peak_finding(y_pred: tf.Tensor, threshold: float = 0.1, volume: List[int] =[5, 13, 13]):
...
def position_precision(y_true: tf.Tensor, y_pred: tf.Tensor):
    ...
    y_pred = tf.where(edges == 0, y_pred, 0)
    ...
    return (tf.reduce_sum(correct_peaks) + 0.001) / (tf.reduce_sum(peaks) + 0.001)
...
def position_recall(y_true, y_pred):
    ...
    y_true = tf.where(edges == 0, y_true, 0)
    y_pred = tf.where(edges == 0, y_pred, 0)
...
def overcount(y_true, y_pred):
    ...
    return (tf.reduce_sum(correct_positions) + 0.001) / (tf.reduce_sum(detected_positions) + 0.001)
