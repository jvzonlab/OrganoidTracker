...
_patch_shape_y = config.get_or_default("patch_shape_y", str(240), type=config_type_int,
                                       comment="Maximum patch size to use for predictions."
...
_model_folder = config.get_or_prompt("model_folder",
                                     "Please paste the path here to the \"trained_model\" folder containing the trained model.")
...
_channels_str = config.get_or_default("images_channels", str(1),
                                      comment="Index(es) of the channels to use. Use \"3\" to use the third channel for predictions. Use \"1,3,4\" to use the sum of the first, third and fourth channel for predictions.")
...
prediction_dataset_all = predicting_data_creator(image_list, time_window, corners,
                                                 patch_shape_zyx, buffer, max_image_shape,
                                                 batch_size=set_size * len(corners))
