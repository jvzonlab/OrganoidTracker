...
from organoid_tracker.linking_analysis.links_postprocessor import postprocess, finetune_solution, connect_loose_ends, \
    bridge_gaps, pinpoint_divisions, remove_tracks_too_deep, bridge_gaps2, remove_tracks_too_short, \
    remove_single_positions, remove_spurs_division
...
for experiment_index, experiment in enumerate(list_io.load_experiment_list_file(_dataset_file)):
    ...
    # finetune solution
    finetune_solution(experiment_all, experiment_result, iterations=4)

    # Connect loose ends, use all available links (experiment) not the pruned set (experiment_all)
    connect_loose_ends(experiment, experiment_result)

    # remove the positions from experiment_all that were removed during connect_loose_ends
    to_remove = []
    ...
    bridge_gaps(experiment_all, experiment_result)

    # connect tracks broken up by a link that is not part of the proposed set
    bridge_gaps2(experiment_all, experiment_result)

    # align the division in tracks so they match the division predictions from the neural network
    pinpoint_divisions(experiment_all, experiment_result)
    remove_spurs_division(experiment_all, experiment_result)
    ...
    remove_tracks_too_deep(experiment_result, max_z=_max_z)
    postprocess(experiment_result, margin_xy=_margin_xy)

    # remove too short tracks
    remove_tracks_too_short(experiment_result, experiment_all, min_t=_minimum_track_length)
    remove_single_positions(experiment_result, experiment_all)
