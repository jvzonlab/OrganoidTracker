...
from organoid_tracker.link_detection_cnn.convolutional_neural_network import focal_loss
...
model = tf.keras.models.load_model(_model_folder,  custom_objects={"focal_loss": focal_loss})
if not os.path.isfile(os.path.join(_model_folder, "settings.json")):
    print("Error: no settings.json found in model folder.")
    exit(1)
...
#print(prediction_dataset_all)

#for i in prediction_dataset_all:
    #print(i)
